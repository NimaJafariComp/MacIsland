...
enum XPCHelperConnectionState: Equatable {
    case unknown
    case available
    case unavailable
}
...
final class XPCHelperClient: NSObject, ObservableObject, @unchecked Sendable {
    ...
    @MainActor
    private func ensureRemoteService() -> RemoteXPCService<MacIslandXPCHelperProtocol> {
        ...
        let service = RemoteXPCService<MacIslandXPCHelperProtocol>(
            connection: conn,
            remoteInterface: MacIslandXPCHelperProtocol.self
        )
    ...
    @MainActor
    private func getRemoteService() -> RemoteXPCService<MacIslandXPCHelperProtocol>? {
        remoteService
    }
    ...
    @MainActor
    private func markServiceAvailable() {
    ...
    nonisolated func isAccessibilityAuthorized() async -> Bool {
        do {
            ...
            await MainActor.run { markServiceUnavailable() }
            return false
        }
    }
    ...
    nonisolated func isKeyboardBrightnessAvailable() async -> Bool {
        do {
            ...
            let available: Bool = try await service.withContinuation { service, continuation in
                service.isKeyboardBrightnessAvailable { available in
                    continuation.resume(returning: available)
                }
            }
            await MainActor.run { markServiceAvailable() }
            return available
        } catch {
            await MainActor.run { markServiceUnavailable() }
            return false
    ...
    nonisolated func setKeyboardBrightness(_ value: Float) async -> Bool {
        do {
            ...
            let success: Bool = try await service.withContinuation { service, continuation in
