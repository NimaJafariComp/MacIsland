...
final class TemporaryFileStorageService {
    ...
    private init() {
        rootDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("MacIslandShelf", isDirectory: true)
        try? FileManager.default.createDirectory(at: rootDirectory, withIntermediateDirectories: true)
        // Shelf temporary data is session-scoped. Clear leftovers from a prior run
        // before persisted bookmarks are restored, preventing dead shelf entries.
        cleanupExpiredFiles()
    }
    ...
    func removeTemporaryFileIfNeeded(at url: URL) {
        guard url.standardizedFileURL.path.hasPrefix(rootDirectory.standardizedFileURL.path + "/") else {
