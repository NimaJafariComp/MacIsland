...
final class ShelfStateViewModel: ObservableObject {
    ...
    private init() {
        // Temporary representations belong to a prior process and are removed by
        // TemporaryFileStorageService at startup. Never restore their dead bookmarks.
        _ = TemporaryFileStorageService.shared
        items = ShelfPersistenceService.shared.load().filter { !$0.isTemporary }
        ShelfPersistenceService.shared.save(items)
    }
