...
class BatteryActivityManager {
    ...
    private var pendingNotifications: [String: BatteryEvent] = [:]
    ...
    /// Coalesces noisy IOKit bursts by event kind. Observers receive latest
    /// values together instead of waiting behind stale queue entries.
    private func enqueueNotification(_ event: BatteryEvent) {
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            self.pendingNotifications[self.notificationKey(for: event)] = event
            guard self.notificationWorkItem == nil else { return }

            let workItem = DispatchWorkItem { [weak self] in
                guard let self else { return }
                let events = self.pendingNotifications.values
                self.pendingNotifications.removeAll()
                self.notificationWorkItem = nil
                events.forEach { self.notifyObservers(event: $0) }
            }
            self.notificationWorkItem = workItem
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.08, execute: workItem)
        }
    }

    private func notificationKey(for event: BatteryEvent) -> String {
        switch event {
        case .powerSourceChanged: return "powerSource"
        case .batteryLevelChanged: return "batteryLevel"
        case .lowPowerModeChanged: return "lowPowerMode"
        case .isChargingChanged: return "charging"
        case .timeToFullChargeChanged: return "timeToFullCharge"
        case .maxCapacityChanged: return "maxCapacity"
